...
async def get_latency(clould_placements, tick, N, k, cloud_providers, data_size, read):
    ...
    async def do_request(cloud_id) -> list:
    ...
    request_tasks = [asyncio.create_task(do_request(cloud_id)) for cloud_id, enabled in enumerate(clould_placements) if enabled == 1]
